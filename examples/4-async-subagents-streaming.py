...
async def test_stream(query):
    """
    异步流式执行一次用户问题，并打印主智能体的调度过程。
    每个 chunk 中，都会输出节点名和完整的消息对象（即大模型的原生返回）。
    """
    print(f"\n====== 开始处理 query: {query} ======")
    stream = main_agent.astream({"messages": [{"role": "user", "content": query}]})

    async for chunk in stream:
        # chunk 是一个字典，如 {"model": {"messages": [...]}}
        for node_name, state in chunk.items():
            if state is None or "messages" not in state:
                continue
            ...
            # 遍历本次节点产出的所有新消息，而不仅仅是最后一条
            for msg in messages:
                # 打印消息的类型和完整内容（原生返回）
                print(f"\n[节点 {node_name}] 收到消息:")
                print(f"消息类型: {type(msg).__name__}")
                # 直接打印消息对象，LangChain 会展示内部字段
                print(f"原始消息: {msg}")
                # 如果需要更结构化的输出，可以转为字典（需引入 from langchain.load import dumpd）
                # 但直接打印 msg 已经足够清晰
                print("-" * 60)
    print(f"====== 处理完成: {query} ======\n")
