...
async def lifespan(_app: FastAPI):
    ...
    logger.info(f"服务启动，WebSocket 事件循环已绑定: {id(loop)}")
    yield
    logger.info("服务关闭")
...
def _forget_task(thread_id: str, task: asyncio.Task) -> None:
    ...
    if active_tasks.get(thread_id) is task:
        ...
        logger.debug(f"任务已从 active_tasks 移除: thread_id={thread_id}")
...
async def run_task(request: TaskRequest):
    ...
    req_logger = logger.bind(thread_id=thread_id)
    req_logger.info(f"收到任务请求: query={request.query[:50]}...")

    # 同一个 thread_id 只保留一个活跃任务，新任务会先取消旧任务，避免并发写同一会话目录
    old_task = active_tasks.get(thread_id)
    if old_task and not old_task.done():
        req_logger.info("取消旧任务")
        old_task.cancel()
    ...
    req_logger.info(f"任务已启动，当前活跃任务数: {len(active_tasks)}")
...
async def upload_files(files: List[UploadFile] = File(...), thread_id: str = Form(...)):
    ...
    """
    req_logger = logger.bind(thread_id=thread_id)
    req_logger.info(f"收到文件上传请求，文件数: {len(files)}")
    ...
    for file in files:
        ...
        req_logger.debug(f"文件已保存: {file.filename}")

    req_logger.info(f"文件上传完成: {saved_files}")
...
async def download_file(path: str):
    ...
    """
    logger.info(f"收到下载请求: {path}")
    try:
        abs_path = Path(path).resolve()
        output_abs = output_dir.resolve()

        if not abs_path.is_relative_to(output_abs):
            logger.warning(f"拒绝访问（路径越界）: {abs_path}")
            return {"error": "拒绝访问: 只能下载输出目录下的文件"}
    except Exception as e:
        logger.error(f"下载路径解析失败: {e}")
...
async def websocket_endpoint(websocket: WebSocket, thread_id: str):
    ...
    """
    ws_logger = logger.bind(thread_id=thread_id)
    ws_logger.info("WebSocket 连接请求")
    ...
    try:
        while True:
            ...
            ws_logger.debug(f"收到心跳: {data}")
            await websocket.send_json(
                {"type": "pong", "message": f"服务端已收到: {data}"}
            )
    except WebSocketDisconnect:
        manager.disconnect(websocket, thread_id)
        ws_logger.info("WebSocket 客户端主动断开")
    except Exception as e:
        ws_logger.error(f"WebSocket 异常: {e}")
        manager.disconnect(websocket, thread_id)
