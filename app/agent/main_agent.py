...
async def run_deep_agent(task_query, session_id):
    ...
    """
    # 绑定 session_id 到日志上下文，后续所有日志自动带上该标签
    task_logger = logger.bind(session_id=session_id)
    task_logger.info(f"任务开始执行 | query={task_query[:100]}...")

    # 每个会话独立使用 output/session_{session_id}，避免不同用户的产物互相覆盖
    session_dir = project_root_path / "output" / f"session_{session_id}"
    session_dir.mkdir(parents=True, exist_ok=True)
    task_logger.debug(f"会话目录已创建: {session_dir}")
    ...
    if updated_dir_path.exists():
        ...
        if files:
            task_logger.debug(f"发现上传文件: {files}")
    ...
    try:
        task_logger.debug("开始 main_agent.astream 流式执行")
                        ...
                        if last_msg.tool_calls:
                            ...
                            for tool_call in last_msg.tool_calls:
                                if tool_call["name"] == "task":
                                    ...
                                    task_logger.info(f"调用子智能体: {subagent_type}")
                            ...
                            task_logger.debug(
                                f"主智能体输出内容片段: {last_msg.content[:100]}..."
                            )
                            monitor.report_task_result(last_msg.content)

        task_logger.info("任务正常结束")

    except asyncio.CancelledError:
        task_logger.warning("任务被用户取消")
        monitor.report_task_cancelled()
        raise
    except Exception as e:
        # 记录完整堆栈，这是定位问题的关键
        error_detail = traceback.format_exc()
        task_logger.error(f"任务执行异常:\n{error_detail}")
